...
| `src/ice/` | Five-man F1/F2/F3/Ds/Dw/G every tick. F1 shoot/pass/clear. | Environment overlay. **Not** a graph. |
| `src/agents/` | Two compiled team graphs. Live macro is Head Coach → assemble. Specialists compiled, opt-in. | `StateGraph` / `compile`, `Command` routing. `Send` fan-out is opt-in. |
