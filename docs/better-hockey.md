...
**Next PR (shipped `6f01c3e`):** AAR prefers 5v5. Evaluate 4 ran.

### Evaluate 4 — `ser-emp-11` (2026-08-26)

AAR 5v5 targeting `6f01c3e`. Same protocol. Twin `ser-emp-11-nollm` event hashes **match** `ser-emp-10-nollm` (AAR PR did not move `--no-llm` ice). Timeout stayed **8000**. xAI returned **429 capacity** mid-series; leftovers used, timeouts not raised.

| Gate | Result |
| --- | --- |
| Control honest | **pass** — v1, retrieveTop 0/6, `booksMoved` false, offsides 0–2 |
| Live writes books | **pass** — v1→v8 both. Home retrieveTop 0/6. Away 4/6 (diagnostic). Wins/losses wrote **122 / breakout / 212**, not umbrella (except g1 tie) |
| Lead-protect skating | **pass** — 0 hits; snapshot games **0** |
| Offsides 0–2 | **pass** — max 2 (g2/g3 home 2) |
| Chance mean | **8.0** (10,8,6,9,9,7,7) ≥ 6 |
| Pairs `--compare 0,6` | **1** < 3 (**floor fail**) |
| Home Δ xG g0→g6 | **−0.376** (raw −0.3762) not **>** −0.376 |

**Improved: no.** Bank **1 / 5**. Flat streak **2**. Attempts **4 / 5**.
...
| --- | --- | --- | --- | --- | --- |
| 0 | 2–3 away | `5v5-122-forecheck` | `oz-crash-net` | 10 / 0 | 3 / 2 |
| 1 | 3–3 tie | `5v5-122-forecheck` | `oz-crash-net` | 8 / 1 | 4 / 1 |
| 2 | 2–0 home | `5v5-122-forecheck` | `oz-crash-net` | 6 / 2 | 1 / 0 |
| 3 | 3–1 home | `5v5-122-forecheck` | `5v5-212-forecheck` | 9 / 2 | 2 / 1 |
| 4 | 2–3 away | `5v5-122-forecheck` | `oz-crash-net` | 9 / 1 | 7 / 1 |
| 5 | 4–3 home | `5v5-122-forecheck` | `5v5-212-forecheck` | 7 / 0 | 4 / 0 |
| 6 | 1–2 away | `5v5-122-forecheck` | `oz-crash-net` | 7 / 0 | 2 / 1 |

Home AAR ops: g0/g4/g6 `add_counter` on **122**; g2/g3 boost **122**; g5 boost **`5v5-breakout-d-to-winger`** (digest topPlay was `pk1-box` 51% — targeting worked); g1 tie still boosted `pp1-umbrella` (share 93%, even-strength share < 0.4).
...
**Next PR (shipped `78b17cb`):** F2 dump-chase. Evaluate 5 ran.

### Evaluate 5 — `ser-emp-12` (2026-08-26)

F2 dump-chase `78b17cb`. Same protocol. Twin `ser-emp-12-nollm` (hashes **moved** vs Evaluate 4 control — ice change). Timeout 8000. Not raised.

| Gate | Result |
| --- | --- |
| Control honest | **pass** — v1, retrieveTop 0/6, offsides 0–2. Control more shots than Evaluate 4 twin |
| Live writes books | **pass** — v1→v8. Home retrieveTop 0/6. Away 1/6 (stretch → crash-net). AAR wrote 122 / breakout |
| Lead-protect skating | **pass** — 0 hits; snapshot games **0** |
| Offsides 0–2 | **fail** — g0 home **3** |
| Chance mean | **5.71** (6,6,7,6,11,2,2) < 6 (**floor fail**) |
| Pairs `--compare 0,6` | **0** < 3 |
| Home Δ xG g0→g6 | **−0.775** not > −0.376 |

**Improved: no.** Bank **1 / 5**. **Cycle abort** (flat 3/3 **and** attempts 5/5).

Live card:

| G | Score | Home retrieve | Away retrieve | Home ch/off | Away ch/off |
| --- | --- | --- | --- | --- | --- |
| 0 | 3–0 home | `5v5-122-forecheck` | `5v5-212-forecheck` | 6 / **3** | 3 / 1 |
| 1 | 2–4 away | `5v5-122-forecheck` | `oz-crash-net` | 6 / 0 | 6 / 0 |
| 2 | 1–0 home | `5v5-122-forecheck` | `oz-crash-net` | 7 / 1 | 1 / 1 |
| 3 | 2–3 away | `5v5-122-forecheck` | `oz-crash-net` | 6 / 1 | 4 / 1 |
| 4 | 1–1 tie | `5v5-122-forecheck` | `oz-crash-net` | 11 / 1 | 6 / 0 |
| 5 | 2–5 away | `5v5-122-forecheck` | `oz-crash-net` | 2 / 1 | 9 / 1 |
| 6 | 2–3 away | `5v5-122-forecheck` | `oz-crash-net` | 2 / 1 | 11 / 1 |

g0 AAR boosted **122** (share 81%). g5/g6 home OZ time **8s / 3s** vs DZ **12s / 27s** — hemmed in. Away Δ xG **+0.655**.
...
**Cycle 2 PR-1 (shipped `66d96d0`):** F2 tighten. Evaluate 6 ran.

### Evaluate 6 — `ser-emp-13` (2026-08-26)

F2 tighten `66d96d0`. Same protocol. Twin `ser-emp-13-nollm`. Timeout 8000.

| Gate | Result |
| --- | --- |
| Control honest | **pass** — v1, retrieveTop 0/6. Control g2 home 3 offs is ice noise on the twin, not a live fail |
| Live writes books | **pass** — v1→v8. Home retrieveTop 0/6. Away 0/6 (crash-net from g0). AAR wrote 122 |
| Lead-protect skating | **pass** — 0 hits; snapshot games **0** |
| Offsides 0–2 | **pass** — g0 home **0** (was 3 on Evaluate 5) |
| Chance mean | **4.57** (9,0,1,6,3,5,8) < 6 (**floor fail**) |
| Pairs `--compare 0,6` | **0** < 3 |
| Home Δ xG g0→g6 | **−0.365** **would** beat −0.376 |

**Improved: no.** Bank **1 / 5**. Cycle 2 attempt 1, flat 1. Δ xG moved; chance mean and pairs did not.

Live card:

| G | Score | Home retrieve | Away retrieve | Home ch/off | Away ch/off |
| --- | --- | --- | --- | --- | --- |
| 0 | 2–2 tie | `5v5-122-forecheck` | `oz-crash-net` | 9 / 0 | 2 / 1 |
| 1 | 1–5 away | `5v5-122-forecheck` | `oz-crash-net` | **0** / 1 | 14 / 1 |
| 2 | 1–2 away | `5v5-122-forecheck` | `oz-crash-net` | 1 / 1 | 7 / 1 |
| 3 | 0–4 away | `5v5-122-forecheck` | `oz-crash-net` | 6 / 0 | 5 / 0 |
| 4 | 0–0 tie | `5v5-122-forecheck` | `oz-crash-net` | 3 / 0 | 8 / 1 |
| 5 | 2–2 tie | `5v5-122-forecheck` | `oz-crash-net` | 5 / 0 | 5 / 0 |
| 6 | 2–3 away | `5v5-122-forecheck` | `oz-crash-net` | 8 / 0 | 4 / 1 |
...
**Cycle 2 PR-5 (shipped `c92df85`):** DZ F1 passes to an outlet ahead of the puck, else clears. Ice pass beats overlay dump only in the DZ. pr8 count 280→**263**, Offside **2→0**, Shot 0, epochs 11. pr7 unchanged. Evaluate 10 ran.

### Evaluate 10 — `ser-emp-17` (2026-08-26)

DZ outlet `c92df85`. Same protocol. Twin `ser-emp-17-nollm` hashes **moved** vs Evaluate 9 twin (ice change). Timeout 8000. Not raised.

| Gate | Result |
| --- | --- |
| Control honest | **pass** — v1, retrieveTop 0/6, `booksMoved` false. Control g3 away **3** offs is twin ice noise, not a live fail |
| Live writes books | **pass** — v1→v8. Home retrieveTop 0/6. Away 0/6 (212) |
| Lead-protect skating | **pass** — 0 hits; snapshot games **0** |
| Offsides 0–2 | **pass** — max 1 live |
| Chance mean | **5.29** (8,4,3,3,8,4,7) < 6 (**floor fail**) |
| Pairs `--compare 0,6` | **3** < 4 (**floor fail**) |
| Home Δ xG g0→g6 | **+0.097** > −0.071 (would credit) |

**Improved: no.** Bank **2 / 5**. **Cycle abort** (attempts 5/5). Flat would have been 2/3.
...
| 1 | 2–3 away | `5v5-122-forecheck` | `5v5-212-forecheck` | 4 / 1 | 7 / 0 |
| 2 | 0–1 away | `5v5-122-forecheck` | `5v5-212-forecheck` | 3 / 0 | 5 / 1 |
| 3 | 2–2 tie | `5v5-122-forecheck` | `5v5-212-forecheck` | 3 / 1 | 6 / 1 |
| 4 | 5–2 home | `5v5-122-forecheck` | `5v5-212-forecheck` | 8 / 1 | 5 / 0 |
| 5 | 3–5 away | `5v5-122-forecheck` | `5v5-212-forecheck` | 4 / 0 | 7 / 1 |
| 6 | 4–1 home | `5v5-122-forecheck` | `5v5-212-forecheck` | 7 / 0 | 6 / 0 |

g0 eventHash **`85fa9942`** matches Evaluate 9 g0. g6 xG **1.015** matches Evaluate 9 g6. Δ xG is the same **+0.097**. g2 is still OZ **6s** / DZ **27s** (same hash as Evaluate 9 g2). The outlet did not unstick the scored games.
...
### Evaluate 12 — `ser-emp-19` (2026-08-26)

F2 established OZ `9ddd4f4`. Same protocol. Twin `ser-emp-19-nollm`. Timeout 8000. Not raised. Coach HTTP leftovers used (not treated as a timeout-raise).

| Gate | Result |
| --- | --- |
| Control honest | **pass** — v1, retrieveTop 0/6, `booksMoved` false. Offsides 0–2 (max 1) |
| Live writes books | **pass** — v1→v8 both. Home retrieveTop 0/6. Still 122 |
| Lead-protect skating | **pass** — 0 hits |
| Offsides 0–2 | **fail** — g5 home **4** (ticks 43 / 21 / 150 / OT-1). g2 home **0** (was 3 on Evaluate 11) |
| Chance mean | **5.14** (4,5,10,5,1,7,4) < 6 (**floor fail**) |
| Pairs `--compare 0,6` | **1** < 4 (**floor fail**). One away-PP DZ goal→goal |
| Home Δ xG g0→g6 | **−0.119** (0.324 → 0.205) not > −0.071 |

**Improved: no.** Bank **2 / 5**. Cycle 3 attempt 2, flat **2 / 3**. One more flat aborts.

Live card:

| G | Score | Home retrieve | Away retrieve | Home ch/off | Away ch/off |
| --- | --- | --- | --- | --- | --- |
| 0 | 1–2 away | `5v5-122-forecheck` | `5v5-212-forecheck` | 4 / 0 | 2 / 1 |
| 1 | 1–1 tie | `5v5-122-forecheck` | `5v5-212-forecheck` | 5 / 1 | 3 / 1 |
| 2 | 1–1 tie | `5v5-122-forecheck` | `5v5-212-forecheck` | 10 / 0 | 2 / 0 |
| 3 | 3–2 home | `5v5-122-forecheck` | `5v5-212-forecheck` | 5 / 1 | 4 / 0 |
| 4 | 1–3 away | `5v5-122-forecheck` | `5v5-212-forecheck` | 1 / 0 | 5 / 0 |
| 5 | 1–1 tie | `5v5-122-forecheck` | `5v5-212-forecheck` | 7 / **4** | 4 / 0 |
| 6 | 2–2 tie | `5v5-122-forecheck` | `5v5-212-forecheck` | 4 / 2 | 6 / 1 |

g0 OZ **17.5s** / 4 chances / 0 offs is better than Evaluate 11’s 8s / 2 chances collapse. g2 10 chances is mostly **`pp1-umbrella`** (12 directives vs 122×1) — volume is PP spray, and g2 AAR boosted umbrella. g4 OZ **4.7s** / DZ **17s**, xG 0.06. g5 tick **43** period-1 home offside is the old tick-46 entry leak on 122, not gone.

Control (`ser-emp-19-nollm`): v1, 0/6, booksMoved false. g0 4–2 xG 0.70. Offs 0–2. Ice hashes moved vs Evaluate 11 twin (g2 4–1 / 10 chances vs old 1–1).

**Diagnose:** F2 `BLUE+8` closed Evaluate 11’s g2 3-off. Remaining entry leak is **F3 racing to `GOAL_LINE-16` the instant `ozLive`**. Just-in OZ F3 is already in the slot while the puck is still nicking the blue. Same established gate as F2. Do **not** revert F2 `BLUE+8`. Do not raise timeouts. Do not change the Δ xG bar. Learning hole (122 retrieve, loser `add_counter`, PP boost on ties) is unchanged; offs band first.

**Cycle 3 next:** F3 holds `ONSIDE_ALONG` until `alongPuck > BLUE+8`. Counting Evaluate 13. Third consecutive flat aborts the cycle.

### Evaluate 13 — `ser-emp-20` (2026-08-26)

F3 established OZ `6a3e6d1`. Same protocol. Twin `ser-emp-20-nollm`. Timeout 8000. Not raised.

| Gate | Result |
| --- | --- |
| Control honest | **pass** — v1, retrieveTop 0/6, `booksMoved` false. Offsides 0–2 (max 2 on g3 away) |
| Live writes books | **pass** — v1→v8 both. Home retrieveTop 0/6. Still 122 |
| Lead-protect skating | **pass** — 0 hits |
| Offsides 0–2 | **pass** — max 1 live (g1/g3/g5/g6 home 1). Evaluate 12’s g5-4 is gone |
| Chance mean | **2.71** (5,3,4,0,5,2,0) < 6 (**floor fail**) |
| Pairs `--compare 0,6` | **1** < 4 (**floor fail**) |
| Home Δ xG g0→g6 | **−0.304** (0.304 → 0.000) not > −0.071 |

**Improved: no.** Bank **2 / 5**. **Cycle abort** (flat 3/3). Attempts 3/5.

Live card:

| G | Score | Home retrieve | Home ch/off | Home OZ/DZ |
| --- | --- | --- | ---: | ---: |
| 0 | 4–2 home | `5v5-122-forecheck` | 5 / 0 | 8s / 13s |
| 1 | 1–1 tie | `5v5-122-forecheck` | 3 / 1 | 5s / 8s |
| 2 | 1–2 away | `5v5-122-forecheck` | 4 / 0 | 21s / 8s |
| 3 | 0–2 away | `5v5-122-forecheck` | **0** / 1 | **2s** / 10s |
| 4 | 1–2 away | `5v5-122-forecheck` | 5 / 0 | 17s / 23s |
| 5 | 2–3 away | `5v5-122-forecheck` | 2 / 1 | 7s / **29s** |
| 6 | 0–3 away | `5v5-122-forecheck` | **0** / 1 | 4s / **31s** |

Away g6 10 chances. Home sat in the DZ. g3/g6 AAR `add_counter` on 122.

Control (`ser-emp-20-nollm`): v1 honest. g0 4–1 xG 0.39. Offs 0–2. g3 home 0 chances too — ice volume is down on both arms.
...
### Evaluate 16 — `ser-emp-23` (2026-08-26)

Leftover retrieve #1 `33878fc`. Same protocol. Twin `ser-emp-23-nollm`. Timeout 8000. Not raised.

| Gate | Result |
| --- | --- |
| Control honest | **pass** — v1, retrieveTop 0/6, `booksMoved` false. Offsides 0–2 |
| Live writes books | **pass** — v1→v8. retrieveTopChanged home **1/6** (cycle → 122) |
| Lead-protect skating | **pass** — 0 hits. g3/g6 protect-lead while leading (legal) |
| Offsides 0–2 | **pass** — max 2 |
| Chance mean | **4.29** (5,4,4,2,2,7,6) < 6 (**floor fail**) |
| Pairs `--compare 0,6` | **0** < 4 (**floor fail**) |
| Home Δ xG g0→g6 | **−0.023** (0.30 → 0.28) not > −0.071 |
| Transfer skate | **g1 opened `nz-122-trap`** (retrieve #1 after 122 had games). Cycle `stats.games` **1**. Then retrieve back to 122 |
...
| G | Score | Home opening | Home retrieve | Home ch/off |
| --- | --- | --- | --- | ---: |
| 0 | 1–2 away | `5v5-122-forecheck` | `oz-cycle-low` | 5 / 1 |
| 1 | 0–0 tie | **`nz-122-trap`** | `5v5-122-forecheck` | 4 / 2 |
| 2 | 1–1 tie | `5v5-122-forecheck` | `5v5-122-forecheck` | 4 / 2 |
| 3 | 2–1 home | `5v5-122-forecheck` | `5v5-122-forecheck` | 2 / 1 |
| 4 | 1–1 tie | `5v5-122-forecheck` | `5v5-122-forecheck` | 2 / 0 |
| 5 | 3–5 away | `5v5-122-forecheck` | `5v5-122-forecheck` | 7 / 1 |
| 6 | 4–2 home | `5v5-122-forecheck` | `5v5-122-forecheck` | 6 / 1 |

g1 trap opening had **4 chances** (Evaluate 15 trap was 0). Unused expired: end menu 122 games 6 net +0.92; cycle games **1** net 0. g1/g4 AAR boosted **`pk1-box`** (even-strength xG empty).
...
**Cycle 5 PR-1 (shipped, counted):** never boost PP/PK when a 5v5 play was on the ice. Pure special-teams games still boost PP/PK. Evaluate 17 ran with Glimmer up.

### Evaluate 17 — `ser-emp-24` (2026-08-27)

AAR no PP/PK boost if 5v5 on ice `99b4e4e`. Same protocol. Twin `ser-emp-24-nollm`. Timeout 8000. Not raised. Glimmer up on `:8080`.

| Gate | Result |
| --- | --- |
| Control honest | **pass** — v1, retrieveTop 0/6, `booksMoved` false. Offsides 0–2. g0 chances 4–5 (same as `ser-emp-23-nollm`) |
| Live writes books | **pass** — v1→v8. retrieveTopChanged home **1/6** (cycle → 122), away **1/6** |
| Lead-protect skating | **pass** — 0 illegal hits. g3×1 / g6×2 protect-lead while leading (legal) |
| Offsides 0–2 | **pass** — max 2 |
| Chance mean | **3.43** (6,4,4,2,2,3,3) < 6 (**floor fail**) |
| Pairs `--compare 0,6` | **1** < 4 (**floor fail**). Pair is `pp1-umbrella` OZ goal→goal |
| Home Δ xG g0→g6 | **−0.049** (0.320 → 0.271) would beat −0.071; floors fail |
| Transfer skate | **g1 opened `nz-122-trap`**. Cycle games **1**. Then retrieve back to 122 |

**Improved: no.** Bank **2 / 5**. Cycle 5 attempt **1 / 5**. Flat **1 / 3**.

Live card:

| G | Score | Home opening | Home retrieve | Home ch/off | Home AAR boost |
| --- | --- | --- | --- | ---: | --- |
| 0 | 2–1 home | `5v5-122-forecheck` | `oz-cycle-low` | 6 / 1 | **`5v5-122-forecheck`** (not umbrella) |
| 1 | 0–0 tie | **`nz-122-trap`** | `5v5-122-forecheck` | 4 / 2 | **`pk1-box`** |
| 2 | 1–1 tie | `5v5-122-forecheck` | `5v5-122-forecheck` | 4 / 2 | `5v5-122-forecheck` |
| 3 | 2–0 home | `5v5-122-forecheck` | `5v5-122-forecheck` | 2 / 1 | `protect-lead-1-1-3` |
| 4 | 1–1 tie | `5v5-122-forecheck` | `5v5-122-forecheck` | 2 / 0 | **`pk1-box`** |
| 5 | 1–4 away | `5v5-122-forecheck` | `5v5-122-forecheck` | 3 / 1 | add_counter `oz-cycle-low` |
| 6 | 3–0 home | `5v5-122-forecheck` | `5v5-122-forecheck` | 3 / 1 | `5v5-122-forecheck` |

g0 mix umbrella×8 + 122×3, digest `topPlay=pp1-umbrella` share 55% — AAR still boosted **122**. That is PR-R5 working when 5v5 had usage seconds. g1 mix trap×1 + cycle×1 + pk-box×11; g4 mix 122×1 + collapse×1 + pk-box×14. Both still boosted **pk1-box**. `evenStrengthOnIce` keys off `playUsage` seconds/xG, not `DirectiveApplied`. Opening leftover that is immediately replaced by a penalty has **0** even-strength seconds, so the strip does not fire.

End menu: 122 games **6** net **+0.44**; cycle games **1** net 0; trap games **1** net 0; pk-box games **4** net **−1.64**.

### Cycle 5 Diagnose (after Evaluate 17)
