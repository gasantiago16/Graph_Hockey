...
export type WsHub = {
  ...
  broadcastTick: (world: WorldState, events: MatchEvent[]) => void;
...
export function createWsHub(opts: { port: number }): WsHub {
  ...
  function pushTick(client: Client, world: WorldState, events: MatchEvent[]): void {
    for (const msg of tickMessages(world, client.inspectSide, events, rosters ? { rosters } : {})) {
      send(client.ws, msg);
    }
  }
  ...
  wss.on("connection", (ws: WebSocket) => {
    ...
    clients.add(client);
    if (latest) pushTick(client, latest, []);
  ...
  return {
    ...
    broadcastTick(world, events) {
      latest = world;
      for (const client of clients) pushTick(client, world, events);
    },
    broadcastStart(msg) {
