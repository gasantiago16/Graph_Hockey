...
**Status:** private repo, **`main` is playable**. Ice F1 actually shoots (one shot per possession). NZ dump-policy now dumps as a live `clear`, not a stick-carry. Live Head Coach finishes the epoch (HC → assemble). AAR rolls match xG into play stats and applies even if the LLM times out. Retrieve excludes lead-protect unless actually leading. `gh series --json` scores distinct chances, offsides, and retrieveTop. Film Room is resimulation; `gh footage --mp4` is a derivative. Handbook: [`docs/FORgasan.md`](docs/FORgasan.md). Experiment log (bank 1/5; Evaluate 3 `ser-emp-10` not credited): [`docs/better-hockey.md`](docs/better-hockey.md).
