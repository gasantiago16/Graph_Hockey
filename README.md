...
**Status:** private repo, **`main` is playable**. Ice F1 actually shoots (one shot per possession). Live Head Coach finishes the epoch (HC → assemble). AAR rolls match xG into play stats and applies even if the LLM times out. Retrieve excludes lead-protect unless actually leading. `gh series --json` scores distinct chances, offsides, and retrieveTop. Film Room is resimulation; `gh footage --mp4` is a derivative. Handbook: [`docs/FORgasan.md`](docs/FORgasan.md). Experiment log (bank 0/5 after `ser-emp-8`): [`docs/better-hockey.md`](docs/better-hockey.md).
